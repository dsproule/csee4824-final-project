...
module rob_testbench;
    ...
    rob dut (
        ...
        .r(r), .T1(T1), .T2(T2),
        .cdb(cdb), .dispatch_valid(dispatch_valid), .NPC(),
        .T(T), .retire_T_out(retire_T), .full(full), .empty(empty),
        .retire(regfile_write_en), .regfile_write_idx_out(regfile_write_idx),
        .V1(V1), .V2(V2), .regfile_write_data(regfile_write_data),
        .rob_table_out(rob_table_out), .commit_NPC(), .ppln_ctrl(), .head(head), .tail(tail)
    );
    ...
    task print_rob;
        ...
        $display("\n(RETIRE) valid: %1b, ROB_T: %4d, flush: %0d regfile idx: %0h\n------------------------------------------", dut.retire, retire_T, dut.ppln_ctrl.flush, regfile_write_idx);
    endtask // print_rob
    ...
    initial begin
        ...
        // === Case 2: Fill ROB ===
        for (int i = tail; i < `ROB_SZ; i++) begin
            dispatch_valid = 1; r = i; expected_r[i-1] = r; @(negedge clock);
        end
        dispatch_valid = 0;
        @(posedge clock);
        ...
        for (int i = 1; i < 6; i++) begin
            cdb.T = i; cdb.V = 4200 + i; expected_V[i-1] = cdb.V;
            cdb.valid = `TRUE;
            @(posedge clock); cdb.valid = `FALSE; @(posedge clock);


            if (!regfile_write_en) exit_on_error("retire signal not high");
            if (regfile_write_data !== expected_V[i-1])
                exit_on_error($sformatf("Commit %0d: data mismatch (got %0d, expected %0d)", i, regfile_write_data, expected_V[i-1]));
            if (regfile_write_idx !== expected_r[i-1])
                exit_on_error($sformatf("Commit %0d: regfile idx mismatch (got %0d, expected %0d)", i, regfile_write_idx, expected_r[i-1]));
            if (retire_T !== i)
        ...
        dispatch_valid = 1; //TODO next one
        ...
        for (int i = 0; i < 15; i++) begin
            ...
            cdb.valid = `TRUE; @(posedge clock); //ready been written, next cycle will have data. Pipelined but latency of a cycle
        ...
        @(negedge clock); cdb.valid = `FALSE; @(posedge clock);
        ...
        dispatch_valid = 0;
        cdb.T = 0; cdb.V = 9999; cdb.ppln_ctrl.flush = 1; cdb.valid = `TRUE;
