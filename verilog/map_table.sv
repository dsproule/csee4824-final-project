...
module map_table (
    ...
    always_comb begin
        ...
        if (cdb.valid && (cdb.T == mt_table[r1].T)) begin
            next_T1 = {mt_table[r1].T, `TRUE};
        end else next_T1 = mt_table[r1]; //functionally conditionals should never both be true

        if (cdb.valid && (cdb.T == mt_table[r2].T)) begin
            next_T2 = {mt_table[r2].T, `TRUE};
        end else next_T2 = mt_table[r2];

        if (retire_entry && (retire_r == r1)) next_T1 = 0;
        
        if (retire_entry && (retire_r == r2)) next_T2 = 0;
    end
