...
module func_unit_1(
    ...
    always_comb begin
        case (S_X_reg.alu_func)
            ALU_MUL:    X_C_packet.result = mult_result[`XLEN-1:0];
            ALU_MULH:   X_C_packet.result = mult_result[2*`XLEN-1:`XLEN];
            ALU_MULHSU: X_C_packet.result = mult_result[2*`XLEN-1:`XLEN];
            ALU_MULHU:  X_C_packet.result = mult_result[2*`XLEN-1:`XLEN];
        endcase
    end
